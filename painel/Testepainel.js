...
function atualizarMelhorOrientacao() {
  ...
  const diffPortrait = Math.abs(aspectImage - aspectGrid);
  const diffLandscape = Math.abs(aspectImage - (rows / cols));
...
genBtn2.addEventListener('click', ()=>{
  ...
  for(let r=0; r<rows; r++){
    ...
    for(let c=0; c<cols; c++){
      ...
      ctx.moveTo(dx-MM_TO_PX(6),dy); ctx.lineTo(dx,dy);
      ctx.moveTo(dx,dy-MM_TO_PX(6)); ctx.lineTo(dx,dy);
      ctx.moveTo(dx+drawW+MM_TO_PX(6),dy); ctx.lineTo(dx+drawW,dy);
      ctx.moveTo(dx+drawW,dy-MM_TO_PX(6)); ctx.lineTo(dx+drawW,dy);
      ctx.moveTo(dx-MM_TO_PX(6),dy+drawH); ctx.lineTo(dx,dy+drawH);
      ctx.moveTo(dx,dy+drawH+MM_TO_PX(6)); ctx.lineTo(dx,dy+drawH);
      ctx.moveTo(dx+drawW+MM_TO_PX(6),dy+drawH); ctx.lineTo(dx+drawW,dy+drawH);
      ctx.moveTo(dx+drawW,dy+drawH+MM_TO_PX(6)); ctx.lineTo(dx+drawW,dy+drawH);
      ctx.stroke();
